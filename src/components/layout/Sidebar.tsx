...
import { PINNED, visibleGroups, isActivePath, type NavItem } from './nav.config';
...
export function Sidebar({ permissions }: { permissions: string[] }) {
  ...
  return (
    <aside className="no-print sticky top-[66px] z-20 hidden h-[calc(100vh-66px)] md:flex w-[68px] shrink-0 flex-col overflow-y-auto border-e border-line bg-surface py-4 lg:w-[236px]">
...
function NavLink({ item, active, counts }: { item: NavItem; active: boolean; counts: NavCounts }) {
  const { t } = useI18n();
